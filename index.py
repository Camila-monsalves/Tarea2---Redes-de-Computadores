...
def eliminar(NOMBRE_CONTACT):
    ...
    if var_NOMBRE_CONTACT == '':
        ...
        buscar = MessageBox.askquestion(
            "Delete alert", "¿Desea eliminar contacto\n" + var_NOMBRE_CONTACT)
        if buscar == "yes":
            return True
        else:
            return False
    ...
    def __init__(self, root):
     ...
     # ---------------- Ventana de la agenda-----------------
        ...
        Label(inbox_frame, text='INGRESE LOS DATOS DEL\n', font=(
            "Calibri", "13", "normal")).grid(row=0, column=0)
        Label(inbox_frame, text='CONTACTO Y SELECCIONE UNA OPCION\n',
              font=("Calibri", "13", "normal")).grid(row=0, column=1)

        Label(inbox_frame, text='NOMBRE_CONTACT', font=(
            "Calibri", "11", "normal")).grid(row=2, column=0)
        inbox_NOMBRE_CONTACT = Entry(inbox_frame, font=(
            "Calibri", "11", "normal"), width=30)
        inbox_NOMBRE_CONTACT.grid(row=2, column=1)
        inbox_NOMBRE_CONTACT.focus()

        Label(inbox_frame, text='TELEFONO', font=(
            "Calibri", "11", "normal")).grid(row=4, column=0)
        inbox_TELEFONO = Entry(inbox_frame, font=(
            "Calibri", "11", "normal"), width=30)
        inbox_TELEFONO.grid(row=4, column=1)

        Label(inbox_frame, text='DIRECCION', font=(
            "Calibri", "11", "normal")).grid(row=6, column=0)
        inbox_DIRECCION = Entry(inbox_frame, font=(
            "Calibri", "11", "normal"), width=30)
        inbox_DIRECCION.grid(row=6, column=1)
        Label(inbox_frame, text='\n\n', font=(
            "Calibri", "11", "normal")).grid(row=7, column=0)

        Label(inbox_frame, text='MENÚ DE LA AGENDA\n', font=(
            "Calibri", "13", "normal")).grid(row=9, column=0)
        # --------------- Boton para añadir un contacto -----------------
        ingresar_button = Button(
            inbox_frame, command = lambda: agregar(), text='Ingrese un contacto', width=20)
        ingresar_button.configure(
            cursor='hand2', font=("Calibri", "10", "normal"))
        # -- da las coordenadas de ubicación del boton--
        ingresar_button.grid(row=10, column=0, padx=2, pady=3, sticky=W + E)

        # --------------- Boton para eliminar un contacto -----------------
        _eliminar_button = Button(
            inbox_frame, command=lambda: _eliminar(), text='Eliminar contacto', width=20)
        _eliminar_button.configure(
            cursor='hand2', font=("Calibri", "10", "normal"))
        # -- da las coordenadas de ubicación del boton--
        _eliminar_button.grid(row=12, column=0, padx=2, pady=3, sticky=W + E)

        # --------------- Boton para mostrar todos los contacto -----------------
        show_contacto_button = Button(inbox_frame, command=lambda: show_contacto(
        ), text='Ver todos los contactos', width=20)
        show_contacto_button.configure(
            cursor='hand2', font=("Calibri", "10", "normal"))
        # -- da las coordenadas de ubicación del boton--
        show_contacto_button.grid(
            row=14, column=0, padx=2, pady=3, sticky=W + E)

        # --------------- Boton para limpiar la pantalla de la agenda -----------------
        guardar_cammbios_button = Button(
            inbox_frame, command=lambda: limpiar(), text='Limpiar vista', width=20)
        guardar_cammbios_button.configure(
            cursor='hand2', font=("Calibri", "10", "normal"))
        # -- da las coordenadas de ubicación del boton--
        guardar_cammbios_button.grid(
            row=16, column=0, padx=2, pady=3, sticky=W + E)

        buscar_button = Button(
            inbox_frame, command=lambda: buscar(), text='Buscar contacto por:', width=20)
        buscar_button.configure(
            cursor='hand2', font=("Calibri", "10", "normal"))
        # -- da las coordenadas de ubicación del boton--
        buscar_button.grid(row=18, column=0, padx=3, pady=3, sticky=W + E)

        combo = ttk.Combobox(inbox_frame, state='readonly', width=17,
                             justify='center', font=("Calibri", "10", "normal"))
        combo["values"] = ['NOMBRE_CONTACT', 'TELEFONO', 'DIRECCION']
        # -- da las coordenadas de ubicación las opciones de busqueda--
        combo.grid(row=18, column=1, padx=2, pady=3, sticky=W + E)
        combo.current(0)

        self.tree = ttk.Treeview(
            three_frame, height=20, columns=("DOS", "TRES"))
        ...
        scroll_x = Scrollbar(
            three_frame, command=self.tree.xview, orient=HORIZONTAL)
        ...
        def visualizar_csv():
            ...
            for i, row in enumerate(contacto):
                ...
                TELEFONO = str(row[1])
                DIRECCION = str(row[2])
                self.tree.insert("", 0, text=NOMBRE_CONTACT,
        ...
        def _check(consulta, var_buscar):
            ...
            if lista_consulta == []:
                ...
                self.tree.insert("", 0, text=NOMBRE_CONTACT,
                                 values=(TELEFONO, DIRECCION))
                self.tree.insert("", 0, text="Buscar resultado por nombre", values=(
                    "Buscar resultado por telefono", "Buscar resultado por dirección"))
        ...
        def agregar():
            ...
            if contact_check == ['', '', '']:
                ...
                self.tree.insert("", 0, text=str(NOMBRE_CONTACT),
                                 values=(str(TELEFONO), str(DIRECCION)))
                self.tree.insert("", 0, text="Nuevo nombre", values=(
                    "Nuevo telefono", "Nueva direccion"))
            contact_check = []
            limpiar_inbox()
